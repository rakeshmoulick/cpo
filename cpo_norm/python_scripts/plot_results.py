...
while k<MaxStp:
    ...
    ax[0].set_xlabel('x')
    ax[0].set_ylabel('$\phi$')    
    ax[0].set_title('$\omega_{pe}t = $'+ str(wpet))
    ax[0].grid(True)
...
# ax[2].plot(x[indx,:], nde[indx,:], 'b--', label='$n_{e1}$')
# ax[2].set_xlabel('x')
...
plt.savefig(pjoin(path,'phiContour.jpg'),dpi=dpi)
# ------------------------------------------------------
fig, ax = plt.subplots(figsize=figsize1/25.4,constrained_layout=True,dpi=ppi)
Z = nde
...
ax.set_xlabel('x')
ax.set_ylabel('$\omega_{pe}t$')
plt.savefig(pjoin(path,'denContour.jpg'),dpi=dpi)
...
# ----------------------------------------
# For rotating gif plots
# for i in range(0,360,1):
#         ax.view_init(elev=10., azim=i)
#         plt.savefig("movie%d.png" % i)
# ----------------------------------------
plt.savefig(pjoin(path,'denSurf.jpg'), dpi=dpi)
